...
<head>
    ...
    <style>
        ...
        .tab-hole { width: 6px; height: 6px; border-radius: 9999px; background: #FBF8F2; border: 1px solid #E7DFCE; }
...
<body class="bg-paper font-body text-ink antialiased">
    ...
    <div class="max-w-5xl mx-auto py-8 sm:py-12 px-4">
        ...
        <!-- Header -->
        <header class="mb-8 sm:mb-10 pb-5 border-b-2 border-ink flex flex-wrap gap-3 justify-between items-end">
            <div>
                <h1 class="font-display font-bold text-2xl sm:text-3xl tracking-tight text-ink">
                    Task Manager
                </h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            ...
            <div class="space-y-6">
                <section class="bg-white p-5 sm:p-6 rounded-lg border border-rule shadow-sm">
                    ...
                    <form action="{{ route('categories.store') }}" method="POST" class="space-y-3">
                        @csrf
                        <input type="text" name="name" placeholder="e.g., Work, Personal, Bills" required
                ...
                <section class="bg-white p-5 sm:p-6 rounded-lg border border-rule shadow-sm">
                    ...
                    <div class="mb-4 pb-3 border-b border-rule">
                        <a href="{{ route('tasks.index', request()->except('category_id')) }}" class="font-mono text-xs uppercase tracking-wide {{ request('category_id') ? 'text-ochre hover:text-ink' : 'text-ink underline underline-offset-2' }}">
                            Show All
                        </a>
                    </div>
                        ...
                        <div class="space-y-2">
                            @foreach($categories as $category)
                                <div class="flex items-center justify-between gap-2 px-2 py-1.5 rounded {{ request('category_id') == $category->id ? 'bg-ochresoft' : '' }}">
                                    <a href="{{ route('tasks.index', array_merge(request()->except('category_id'), ['category_id' => $category->id])) }}" class="flex items-center gap-2 text-sm font-medium {{ request('category_id') == $category->id ? 'text-ochre' : 'text-inksoft hover:text-ink' }}">
            ...
            <div class="md:col-span-2 space-y-6">
                ...
                <section class="bg-white p-5 sm:p-6 rounded-lg border border-rule shadow-sm">
                    ...
                    <form action="{{ route('tasks.store') }}" method="POST" class="flex flex-col sm:flex-row gap-3">
                        ...
                        <input type="date" name="due_at"
                            class="rounded border border-rule px-3 py-2 text-sm text-inksoft bg-paper focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                ...
                <form method="GET" action="{{ route('tasks.index') }}" class="flex flex-col sm:flex-row gap-3">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search tasks..."
                        class="flex-1 min-w-0 rounded border border-rule px-3 py-2 text-sm bg-white focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                    <select name="sort" class="rounded border border-rule px-3 py-2 text-sm bg-white text-inksoft focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                        <option value="created_at" @selected($sort === 'created_at')>Newest First</option>
                        <option value="due_at" @selected($sort === 'due_at')>By Due Date</option>
                        <option value="title" @selected($sort === 'title')>By Title</option>
                    </select>
                    <select name="direction" class="rounded border border-rule px-3 py-2 text-sm bg-white text-inksoft focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                        <option value="desc" @selected($direction === 'desc')>Desc</option>
                        <option value="asc" @selected($direction === 'asc')>Asc</option>
                    </select>
                    <button type="submit" class="px-4 py-2 rounded text-sm font-medium text-white bg-ink hover:bg-ink/90 transition">
                        Apply
                    </button>
                </form>
                ...
                <main>
                    <ul role="list" class="space-y-2">
                        ...
                        @foreach ($tasks as $task)
                            <li class="task-card bg-white border border-rule rounded-lg px-5 py-4 shadow-sm">
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                    <div class="flex items-start sm:items-center flex-1 min-w-0 gap-4">
                                        <form action="{{ route('tasks.toggle', $task) }}" method="POST" class="pt-0.5 sm:pt-0">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <div class="min-w-0">
                                            <div class="flex flex-wrap items-center gap-2">
                                                ...
                                                @endif
                                                @if($task->due_at)
                                                    <span class="font-mono text-[10px] uppercase {{ ! $task->is_completed && $task->due_at->isPast() ? 'text-rust' : 'text-inksoft/60' }}">
                                            ...
                                            <details class="mt-1">
                                                <summary class="cursor-pointer font-mono text-[10px] uppercase text-inksoft/40 hover:text-inksoft select-none">Edit</summary>
                                                <form action="{{ route('tasks.update', $task) }}" method="POST" class="mt-2 flex flex-wrap gap-2 items-center">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="text" name="title" value="{{ $task->title }}" required
                                                        class="text-xs rounded border border-rule px-2 py-1 flex-1 min-w-[140px] bg-paper focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                                                    <input type="date" name="due_at" value="{{ optional($task->due_at)->format('Y-m-d') }}"
                                                        class="text-xs rounded border border-rule px-2 py-1 bg-paper focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                                                    <select name="category_id" class="text-xs rounded border border-rule px-2 py-1 bg-paper focus:border-ochre focus:ring-1 focus:ring-ochre outline-none">
                                                        <option value="">No Label</option>
                                                        @foreach($categories as $category)
                                    ...
                                    <form action="{{ route('tasks.destroy', $task) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this task?');" class="flex-shrink-0 self-end sm:self-auto">
                                        @csrf
                                        @method('DELETE')
